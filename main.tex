...
\usepackage[style=numeric, sorting=none, firstinits=true, terseinits=true, minnames=3, mincitenames=1, maxcitenames=3, maxnames=1]{biblatex}
...
prioritizing  for intervention solely based on patterns of transmission inferred from HIV sequence data~\supercite{Bbosa2019,Villandre2019,Oster2018,Ragonnet-Cronin2019,Wertheim2018,Wertheim2011,Wertheim2014,Smith2009}.
...
\textcite{Wertheim2018} present a method for prioritizing \PLWH{s} based on performing transmission clustering (i.e., grouping individuals with low viral genetic distance into \textit{transmission clusters}) and ordering clusters  by growth rate.
...
\textcite{Moshiri2018} have studied the same question in simulations and have shown that monitoring cluster growth can be used for predicting future transmissions substantially better than a random guess, whether clusters are defined using genetic distances or using phylogenetic methods. 
Most recently, \textcite{Balaban2019}  showed in simulations that using a cluster-monitoring approach similar to that of \textcite{Wertheim2018}  but defining clusters  using a min-cut optimization problem gives a small but consistent improvement over defining clusters using genetic distances. 
...
For the most part, we use the base parameters used in \textcite{Moshiri2018} that sought to model the San Diego HIV epidemic from 2005 to 2014, with the following modifications to better capture reality. See Table~S2 for the full set of parameters of the default condition.

\subsubsection{Sexual contact network---}
%In \textcite{Moshiri2018}, sexual contact networks were simulated under the Barab\`asi--Albert (BA) model \supercite{Barabasi1999} such that the expected degree (i.e., average number of sexual contacts for a single individual) was 4.
To capture the scale-free nature of the sexual contact network, \textcite{Moshiri2018} used the Barab\`asi--Albert (BA) model \supercite{Barabasi1999}.
...
similar to that proposed by \textcite{Wertheim2018},
...
Using molecular epidemiology for prioritization is, of course, not a new idea. For example, \textcite{Wertheim2018} presented a method to prioritize \PLWH{s} based on the growth rates of their transmission clusters.
\textcite{Vasylyeva2018} performed a phylogeographic analysis to reconstruct HIV movement among different locations in Ukraine in order to infer region-level risk prioritization.
Much earlier even, \textcite{Mellors1996} predicted HIV patient prognosis by quantifying HIV RNA in plasma; predicted prognosis can subsequently be used as a prioritization rank.
